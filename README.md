...
3. **Important:** `apps/mobile/.env` is gitignored, so EAS Build's upload never includes it — a build without the next step will silently fall back to `http://localhost:3000` (useless on a real phone). Register the var with EAS directly instead:
   ...
   ```
   This is a one-time step per project, not per build — once registered, every future `eas build` reuses it automatically. If you run it again and get `Variable EXPO_PUBLIC_API_URL already exists on this project`, that means it's already set from a previous run; skip this step unless your LAN IP has changed, in which case re-run with `--force` added to the command above to overwrite the stored value.
...
- **On WSL2, Postgres doesn't auto-start on boot** (no systemd by default), so a cluster that was working in a previous session can simply be stopped after a WSL/Windows restart. `npm run backend:dev` fails fast in this case with `PrismaClientInitializationError: Can't reach database server at localhost:<port>` right after Nest finishes mapping routes — it looks like a config problem but usually just means Postgres isn't running. Check with `pg_lsclusters` (look for `Status: down`) and start it with `sudo pg_ctlcluster <version> main start` (e.g. `sudo pg_ctlcluster 12 main start`) before re-running the backend.
